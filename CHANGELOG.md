...
_Dated history: session logs, shipped features, slice/commit tables, and resolved roadmap detail,_
_split out of STATUS.md (which became an 896-line append-only log). Newest first. STATUS.md now_
_holds only the current state; this file is the archive._
...
### Character pipeline — 2D art-bible → 3D → auto-rig (ACTIVE)
The whole character workflow now lives IN Crucible (memory `[[project-crucible-character-3d-rig]]`): LD-forge-parity 2D
...
- **THE PIVOT:** the hand-rolled Blender skinner (geometric nearest-bone + Laplacian
  ...
  on Replicate). **PROVEN:** on the exact boss that broke the old rig, UniRig's skin deforms as a coherent
  volume (no taffy); on a full humanoid it produced spine+neck+head, both arms **with fingers**, both legs
  **with toes** — richer than the old fixed 19-bone armature. UniRig gives skeleton+skin but **no clips**.
...
- **Pipeline wired end-to-end (cont.):** `auto-rig.mjs` now DEFAULTS to the UniRig engine
...
- **Learning — riggability is set upstream, not by a TRELLIS flag.** You can't make TRELLIS emit
  rig-ready topology; riggability is ~entirely the 2D input pose + the normalize step. The contract:
...
### GYRE-retro opportunities — "make the kit make games" (SHIPPED)
Full analysis: `docs/GYRE-RETRO.md`. GYRE (web-projects/gyre), the first real game built ON the kit, exposed the kit as
...
### GYRE → 15-min philosophical tech demo (in progress)
Spec: `gyre/docs/GYRE-DEMO.md`. Ship GYRE: splash+settings,
...
→ 3 endings. Build COMBAT-FIRST. C1 standalone battle (engine + `useSceneMachine` + SMT-style
...
### Anti-sameness + 2D-mobile arm (audit; memory `[[project-2d-mobile-kit-gap]]`)
Two Director goals: make scaffolded games feel DIFFERENT, and be able to make 2D mobile games (puzzle/runner/rhythm).
...
### North star (Director, 2026-06-30) — historical framing, see STATUS.md for current
...
### game-kit npc — Wayfinders NPC brain
...
### biome Place tab + NPC demo + sample game
...
### design-brief generator + biome editor + kit adoption
...
### NPC memory v2 + behavior + deploy fix
...
- **Open (at the time):** transformers.js local-model embedder (dep call), B5 reasoning→behavior bridge (gated), and the
  design-brief generator (Anthropic-backed Architect → brief → scaffolder).

### scaffolder v2 — templates: multiplayer + procgen-world
...
### kit r3f variants + /kit scaffolder
...
### game-kit complete — camera/render/presets/fx/net/clip
...
### game-kit input/save/math + Kit health-check
...
### foundational kit trio + Roblox Phase 2
...
### r3f kit variants + asset-system v2.1 + Roblox Phase 1
...
### asset-system v2 + game-kit Phase-2
...
### asset-systems + game-kit + Roblox onboard
...
### animation viewer + scene composer + deceive-me-daddy
...
### import-from-GitHub + object editor + more games
...
### bulk grab + splash hero + responsive
...
### live library + grab-from-Wayfinders
...
### Smaller follow-ups (as of 2026-06-29)
...
## Phase history (slices, gates, roadmap notes predating daily logs)
...
**Last green gate (as of this phase):** typecheck 0 · lint 0 · **test 75** · build 0. Routes: `/`, `/generate`, `/review`, `/canon`, `/intake`, `/prompts`, `/library`, `/projects/[slug]`, `/projects/new`, `/api/import`.
...
### Style-fidelity gap (historical — first identified here)
...
> **Reference brought in:** the **Living Dungeon (March)** art bible/prompts
> scaffolded *really good* assets. Its scaffolding structure (incl. its
> `GAME_PROFILES`/`buildSystem*` prompts, local-only at `_reference/march-asset-forge/`) was studied and
> folded into the canon engine.

Three options considered, cheapest first:
...
2. **Nano Banana (Gemini 2.5 Flash Image)** — add a Google adapter alongside FLUX in the executor
   ...
   to the LoRA. Tried before committing to the LoRA — shipped as the Nano Banana provider above.
3. **The LoRA slice** (Replicate, Path A per `CANON_INTAKE.md` §5) — the real enforcer. Train on a
   **turntable-render dataset** of the game's actual faceted assets (~15–40 neutral shots/concept;
   ...
   `recipe_snapshot`. `canons.lora_*` fields + trigger `wyfndrstyle` already reserved. Deferred — see
   memory `[[project-no-lora-in-reference]]`: no LoRA code ever existed in the reference; the pipeline
   would be build-fresh.

## Long-range roadmap notes (undated additions, mostly still open — see STATUS.md for current priority)

_These roadmap sections accumulated as dated "added 2026-06-29" notes appended to STATUS.md over_
_several sessions. Anything still live is summarized in STATUS.md's Arm sequencing section; this is_
_the full unabridged detail for reference._
...
### Live procgen renderer, evolutionary (added 2026-06-29)
(1) NOW — harness exports each runtime mesh via GLTFExporter
...
of starting from scratch. Batch worker itself: (deferred at the time — mobile is mostly review/library, not
creation): resumable, cost-capped, enables remote/bulk generation. Build when remote creation becomes the
need. (Since shipped — see Phase 3 slice 1/2 above.)
...
for any asset that CARRIES clips. Open question (being explored at the time): project-mmo's anims are PROCEDURAL
...
procedural anims is the follow-on per-game export work. (Player shipped — see animation viewer above.)
...
catalogued/scaffolded by Crucible. Decide package layout before extracting. (Since shipped as game-kit.)

#### game-kit — built vs gaps (audit 2026-06-29; live view at `/kit`)
**Built — 20 systems (at the time of this audit)** (`github.com/kalogan/game-kit`): prng · settings · scene-state · lighting (+r3f) ·
...
input/save/math, and the remaining camera/render/netcode/fx/skeletal/presets). **Remaining frontier at the time:**
- **r3f variants** of the new vanilla systems (camera/render/fx) for the 2 r3f games (only lighting+postfx
  had r3f variants at the time).
- **LOW** — i18n · a11y filters.
- The real wins then: **adopt the kit in a real game** (validate the API end-to-end — since done via GYRE),
  and grow `/kit` from a dashboard → a **scaffolder** ("pick pieces → new game" — since shipped).

The **`/kit` health-check** page shows the live adoption matrix (system × game: uses-own / opportunity /
n-a) + "build next / adopt here / expand to" rankings.

**Recommended next steps (at the time of the audit):**
1. Add the **foundational trio** (art-kit registry + geometry/material helpers + palette) — unlocks procgen
   parity + the Roblox renderer in one move (highest leverage). (Shipped — see foundational kit trio above.)
2. **Input + camera rigs** next. (Shipped.)
3. **Publish + version** it (set `exports` for `.`/`./r3f`, add a `tsc→dist` build) so games can install it;
   ...
   the dual vanilla/r3f API. (Superseded — GYRE became the adoption proof instead.)
4. **Crucible as the catalog/scaffolder** — a page listing kit pieces (copy/scaffold), → a "new game" generator. (Shipped.)
...
**Animation baking (was in progress):** bake procgen games' PROCEDURAL animators (project-mmo —
...
existed at the time**. Conversions: studs↔m (~1 stud≈0.28m), `Color3`↔hex, `CFrame`↔quat+pos, Material enum↔PBR.
...
- **Ingest (Roblox→Crucible):** read the agnostic descriptors (JSON) → a `descriptor→three` builder
  ...
  the fallback. (Shipped — Roblox Phase 1/2 above.)
- **Port web→Roblox:** a `descriptor→Luau/rbxm` emitter — author/edit in Crucible (scene editor +
  composable asset-systems) → export Roblox instances. "Port from here with ease." (Shipped — Luau emit above.)
...
Where `game-kit/npc` was at the time: dialogue brain (provider seam + Grok + firewall), budgeted, and a
...
Behavior EXISTED in Wayfinders but in **sim-core** (`tickNpcBehavior` + nav `Pathfinder` + bounds
...
- **A3 — Embeddings + semantic recall** SHIPPED (game-kit `749b1ff`): `Embedder` seam +
  ...
  **Remaining opt-in (at the time):** a real local MODEL (transformers.js all-MiniLM ~25MB) is a one-file `Embedder`
  adapter — that dep call still stood; the lexical default shipped so the whole recall path worked. (Since
  shipped as the optional transformers.js embedder above.)
- **A4 — Consolidation / forgetting** (polish, still open): periodic merge of near-duplicate memories + recency×salience
...
- **B5 — Reasoning↔behavior bridge (the careful boundary widening):** SHIPPED — gated behind
  `allowMovement:true` (default off), firewall-clamped `goTo`/`emote` intents, runtime goal-injection
  (`requestGoTo`/`emote`/`onIntent`). The deterministic layer validates + plans + executes; the LLM still
  never sets a position. See the B5 entries in the 2026-06-30 section above for the full ship log.
...
**SHIPPED so far (as of 2026-06-30):** A1·A2·A3 + B1·B2·B3·B4 all landed (game-kit `7a52cf2`·`749b1ff`·`439950d`), plus
B5 (gated). Remaining open item: the real local-MODEL embedder is opt-in only (transformers.js, shipped as an
optional sub-entry, not the default).

### Roadmap — game-kit "make a real game" gaps + design-brief generator (added 2026-06-29)
The kit had 25 systems at the time (incl. nav/behavior/npc). To stand up a SAMPLE GAME end-to-end, the gaps were:
- **A composition/glue layer** — the kit ships seams; a sample needs a thin "wire render+input+camera+
  behavior+nav into a loop" example. The scaffolder's procgen-world template was the seed. (Since extended —
  see the GYRE-retro glue-harvest above.)
- **Collision/physics-lite** (movement vs. world), an **entity/ECS-lite** registry, **assets/loading**
  (GLB via the existing viewer), and a **game-state/objective** loop — none were in the kit at the time.
- **Design-brief generator** — shipped (see design-brief generator entry above): idea → Architect agent →
  structured brief → feeds the scaffolder.

### Later phases (as originally planned; see STATUS.md for current state)
- **Phase 3 — bulk + finish + publish:** resumable, cost-capped **batch worker** (sync gen is
  prod-unsafe at volume) — SHIPPED (slices 1+2 above); **Kiln** finishing module (retopo + baked PBR) —
  still open; **CDN publish** + per-project manifest (Supabase Storage → R2 when distribution matters) —
  still open.
- **Phase 4 — two-game proof:** the **deception-game train station** as a second canon (Path B — intake
  ...
  Still open.
- **Phase 5 — avatars** (originally deferred): superseded — shipped as the character pipeline (2026-07-01),
  see that section above. This line is retired.

### Roadmap — game-kit "make a real game" gaps, added detail (2026-06-29)
...
- **Borrow from `threejs-game-skills`** (audit → `docs/EXTERNAL-SKILLS-AUDIT.md`, MIT). Not started as of
  2026-07-01 — Kevin exploring the current build first. When ready, in priority order: **(T1)** port the
  provider-agnostic quality checklists (AAA visual scorecard, definition-of-done, playtest-QA, release-risk)
  into `/brief` + scaffolder + review; **(T2)** physics adapter seam (Rapier) + headless-canvas QA (Playwright,
  new dep) + touch/safe-area HUD + perf overlay — real game-kit gaps; **(T3, paid/optional)** Tripo (auto-rig +
  animation — what TRELLIS can't) + ElevenLabs (real SFX/voice) adapters behind the executor. **Meta:** package
  game-kit + scaffolder + `/brief` as composable agent skills ("crucible game skills") — a framework play.
